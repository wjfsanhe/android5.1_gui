...
Surface::~Surface() {
    ...
    ALOGD("~Surface called");
    if (mConnectedToCpu) {
	ALOGD("~Surface call disconnect");
...
int Surface::queueBuffer(android_native_buffer_t* buffer, int fenceFd) {
    ...
    ALOGE("Surface::queueBuffer,Slot %d",i);
    if (i < 0) {
        return i;
    }
        ...
        ANativeWindow_Buffer* outBuffer, ARect* inOutDirtyBounds)
{
    ...
    if (!mConnectedToCpu) {
    	...
    	ALOGE("Surface::begin lock Surface");
...
#endif
    	ALOGE("Surface:: connect OK");
    }
    ANativeWindowBuffer* out;
    int fenceFd = -1;
    status_t err = dequeueBuffer(&out, &fenceFd);

    ALOGE("Surface:: dequeueBuffer Return 0x%x",err);
    ALOGE_IF(err, "dequeueBuffer failed (%s)", strerror(-err));
    if (err == NO_ERROR) {
        sp<GraphicBuffer> backBuffer(GraphicBuffer::getSelf(out));
        const Rect bounds(backBuffer->width, backBuffer->height);
	ALOGD("Buffer:[%d x %d]",backBuffer->width,backBuffer->height);
	...
	ALOGD("backBufferSlot: idx %d",backBufferSlot);
	...
	ALOGD("before LockAsync");
        ...
        ALOGW_IF(res, "failed locking buffer (handle = %p)",
                backBuffer->handle);
	ALOGD("backBuffer LockAsync success");
        if (res != 0) {
